...
# -*- coding: utf-8 -*-
"""
Workflow Runner with Standard Labware

This script runs a workflow using only standard labware to ensure all OT-2 movements work correctly.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("workflow_standard_labware.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
LOGGER = logging.getLogger("WorkflowStandardLabware")

def run_workflow_standard_labware(workflow_file: str, robot_ip: str = None, arduino_port: str = None):
    """Run a workflow using only standard labware."""
    ...
    # Create a new workflow with only standard labware
    LOGGER.info("Creating a new workflow with only standard labware...")
    
    standard_workflow = {
        "name": "Standard Labware Workflow",
        "version": "1.0.0",
        "description": "A workflow using only standard labware",
        "global_config": {
            "labware": {
                "reactor_plate": {
    ...
    # Save the standard workflow to a temporary file
    temp_workflow_file = "standard_workflow.json"
    with open(temp_workflow_file, 'w') as f:
        json.dump(standard_workflow, f, indent=2)
    
    LOGGER.info(f"Saved standard workflow to {temp_workflow_file}")
    
    # Create the workflow executor
    LOGGER.info(f"Creating WorkflowExecutor with standard workflow file: {temp_workflow_file}")
    ...
    LOGGER.info("Executing workflow with standard labware...")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run a workflow with standard labware")
    ...
    success = run_workflow_standard_labware(args.workflow_file, args.ip, args.port)
