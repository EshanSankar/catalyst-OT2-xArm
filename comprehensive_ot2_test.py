...
"""
Comprehensive test script for OT2 robot connectivity.
This script tests various API endpoints and provides detailed information about the robot.
"""
...
logger = logging.getLogger("OT2Test")
...
def test_system_info_endpoint(ip_address, timeout=5):
    """Test the system info endpoint of the OT2 robot."""
    base_url = f"http://{ip_address}:31950"
    system_info_endpoint = f"{base_url}/robot/settings"
    headers = {"opentrons-version": "3"}

    logger.info(f"Testing system info endpoint at {system_info_endpoint}...")

    try:
        response = requests.get(system_info_endpoint, headers=headers, timeout=timeout)

        if response.status_code == 200:
            logger.info("System info endpoint test PASSED")
            return response.json()
        else:
            logger.error(f"System info endpoint test FAILED: Status code {response.status_code}")
            return None
    except Exception as e:
        logger.error(f"System info endpoint test FAILED: {str(e)}")
        return None
...
def display_robot_info(health_info, system_info, pipettes_info, modules_info):
    ...
    if system_info:
        print("\nSYSTEM INFORMATION:")
        print(f"  CPU Usage: {system_info.get('cpu_usage', 'N/A')}%")
        print(f"  Memory Usage: {system_info.get('memory_usage', 'N/A')}%")
...
def main():
    ...
    logger.info(f"Starting comprehensive OT2 test for robot at {ip_address}")

    # Test all endpoints
    health_info = test_health_endpoint(ip_address, args.timeout)
    system_info = test_system_info_endpoint(ip_address, args.timeout)
    ...
    if health_info:
        display_robot_info(health_info, system_info, pipettes_info, modules_info)
